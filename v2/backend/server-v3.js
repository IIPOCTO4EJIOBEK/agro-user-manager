...
const app = express();
const PORT = 4002; // V3 runs on different port
...
// --- SQLite Audit ---
const AUDIT_DB = '/opt/agro-user-manager-v2/data/audit.db';
...
app.post('/api/create-user', isAuth, [
    ...
    try {
        ...
        await ldapAdd(req.session.adUrl, req.session.adUser, req.session.adPass, userDN, entry);
        
        // Enable + set password
        const pwdBase64 = Buffer.from(`"${pass}"`, 'utf16le').toString('base64');
        await ldapModify(req.session.adUrl, req.session.adUser, req.session.adPass, userDN, [
        ...
        if (d.addVpn) {
            try {
                await ldapModify(req.session.adUrl, req.session.adUser, req.session.adPass,
...
app.post('/api/save-user', isAuth, async (req, res) => {
    ...
    const changes = [];
    const attrMap = {
        displayName: 'displayName', title: 'title', department: 'department', company: 'company',
        l: 'l', st: 'st', mail: 'mail', info: 'info',
        telephoneNumber: 'telephoneNumber', mobile: 'mobile', homePhone: 'homePhone',
        extensionAttribute1: data.sn, extensionAttribute2: data.givenName, extensionAttribute3: data.patronymic,
        assistant: 'assistant', description: 'description', physicalDeliveryOfficeName: 'office'
    };
    ...
    if (data.newOU && data.newOU !== dn.substring(dn.indexOf(',') + 1)) {
        ...
        try {
            await ldapModify(req.session.adUrl, req.session.adUser, req.session.adPass, newDN, changes);
    ...
    try {
        await ldapModify(req.session.adUrl, req.session.adUser, req.session.adPass, dn, changes);
...
app.post('/api/batch-toggle', isAuth, async (req, res) => {
    ...
    for (const dn of dns) {
        try {
            await ldapModify(req.session.adUrl, req.session.adUser, req.session.adPass, dn, [
...
app.post('/api/vpn-add', isAuth, async (req, res) => {
    ...
    try {
        ...
        await ldapModify(req.session.adUrl, req.session.adUser, req.session.adPass, vpnGroup,
...
app.post('/api/compare-csv', isAuth, (req, res) => {
    try {
        ...
        const raw = parse(txt, { columns: true, delimiter: ';', skip_empty_lines: true, relax_column_count: true });
        const results = [];
        for (const r of raw) {
            const inc = {
                ...
                description: r['Описание'], office: r['Адрес'], manager: r['Менеджер'] || '', siteOU: r['siteOU'] || ''
            };
...
process.on('uncaughtException', (err) => {
    ...
    try { fs.appendFileSync('/opt/agro-user-manager-v2/logs/app.log', new Date().toISOString() + ' [CRASH] ' + err.message + '\n' + err.stack + '\n'); } catch(e) {}
});
process.on('unhandledRejection', (reason, promise) => {
    console.error('UNHANDLED REJECTION:', reason);
    try { fs.appendFileSync('/opt/agro-user-manager-v2/logs/app.log', new Date().toISOString() + ' [REJECTION] ' + reason + '\n'); } catch(e) {}
});
    ...
    for (const action of actions) {
        try {
            if (action.type === 'update' && action.dn) {
                const changes = [];
                const d = action.data || {};
                const attrMap = { displayName: 'displayName', title: 'title', department: 'department', company: 'company',
                    l: 'l', st: 'st', mail: 'mail', info: 'info', telephoneNumber: 'telephoneNumber', mobile: 'mobile',
                    homePhone: 'homePhone', extensionAttribute1: d.sn, extensionAttribute2: d.givenName, extensionAttribute3: d.patronymic,
                    assistant: 'assistant', description: 'description', physicalDeliveryOfficeName: 'office' };
                for (const [k, v] of Object.entries(attrMap)) {
                ...
                if (changes.length) await ldapModify(req.session.adUrl, req.session.adUser, req.session.adPass, action.dn, changes);
