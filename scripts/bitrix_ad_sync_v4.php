<?php
// bitrix_ad_sync_v4.php - ИДЕАЛЬНАЯ СТРУКТУРА С РЕАЛЬНОЙ ВЛОЖЕННОСТЬЮ
...
// 1. ИМПОРТ СТРУКТУРЫ (РЕКУРСИВНО)
$search = ldap_search($ldap, $CFG['base_dn'], "(objectClass=organizationalUnit)", ['ou']);
$entries = ldap_get_entries($ldap, $search);
$bs = new CIBlockSection();
$ou_to_id = [strtolower($CFG['base_dn']) => 1];

// Сортируем по глубине DN (количеству запятых), чтобы родители создавались раньше
...
foreach ($entries as $entry) {
    ...
    $name = $entry['ou'][0];
    $parent_dn = preg_replace('/^ou=[^,]+,/i', '', $dn);
...
do {
    ...
    for ($i=0; $i < $entries['count']; $i++) {
        ...
        $parent_dn = preg_replace('/^cn=[^,]+,/i', '', $dn);
        $dept_id = $ou_to_id[$parent_dn] ?? 1;
...
echo "Finished. Correct structure and users: $total\n";
